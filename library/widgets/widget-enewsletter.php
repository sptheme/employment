...
add_action( 'widgets_init', 'sp_enewsletter_widget' );
function sp_enewsletter_widget() {
...
class sp_widget_enewsletter extends WP_Widget {
	...
	*/
	...
	function widget( $args, $instance) {
		...
		$out .= '<form class="subscribe-form" action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open(\'http://feedburner.google.com/fb/a/mailverify?uri=' . $feed_address . '\', \'popupwindow\', \'scrollbars=yes,width=550,height=520\');return true">';
		$out .= '<label for="mail_subscriber">' . $feed_desc . '</label>';
		$out .= '<div><input type="text" class="mail-subscriber" name="mail" placeholder="Enter your email…"></div>';
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Email subscription', 'feed_desc' => 'Want to be notified when news is updated? Enter your email address to be the first to know.', 'feed_address' => 'EurasieTravel');
